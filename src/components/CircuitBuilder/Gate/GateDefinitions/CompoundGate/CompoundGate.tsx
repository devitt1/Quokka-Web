...
const CompoundGate: React.FC<CompoundGateProps> = (props) => {
    ...
    return (
        <OverlayTrigger overlay={(width > height ?
            <Tooltip id="button-tooltip2" className={styles.toottiptext}>{gateName}</Tooltip> :
            <Tooltip style={{display: "none"}}/>)} placement="right">
            <g className={styles.compoundGate}>
                <rect className={styles.rectBkg} width={width} height={height} rx="4"/>
                <AdaptiveTextBox
                    text={(gateName.length > 2 && width > height) ? gateName.slice(0, 2) + "..." : gateName}
